...
initial_ss <- function(A, beta, theta){
  with(as.list(c(A, beta, theta)),{
    ...
    return(list('w'= w, 'C'= C, 'N'= N, 'PIB'= PIB, 'ns'= N, 'ni'= N, 'nr'= N, 'U'= U, 'A'= A, 'beta'= beta, 'theta'= theta, 'Neff'= N,'D_ss'= D_ss, 'taxn_ss' = taxn_ss, 'd_ss'= d_ss))
  }
  )
}
...
td_eval <- function(ns, ni, nr, ctax, he, U_ss, c_ss, n_ss, pr_treat, pr_vacc, pi1, pi2, pi3, H, eps, pidbar, pir, phi, theta, A, beta, kappa){
  with(as.list(c(ns, ni, nr, ctax, he, U_ss, c_ss, n_ss, pr_treat, pr_vacc, pi1, pi2, pi3, H, eps, pidbar, pir, phi, theta, A, beta, kappa)),{
    ...
    #he_arbitr <- readRDS("he_arbitrario.rds")
    #he = he_arbitr[1:H]
    
    b_ss = 892.0274
    taxn_ss = (1/beta -1) * 0.8
    ...
    b[1] = b_ss + (0 + ((1/beta) - 1) * b_ss - taxn_ss * A * (S[1] * ns[1] + I[1] * ni[1] * phi + R[1] * nr[1]))/(1 + gama * A * (S[1] * ns[1] + I[1] * ni[1] * phi + R[1] * nr[1]))
    #b[1] = ((S[1] + I[1] + R[1])*(0 + (1/beta) * b_ss) + (gama * b_ss - taxn_ss) * A * (S[1] * ns[1] + I[1] * ni[1] * phi + R[1] * nr[1]))/((S[1] + I[1] + R[1]) + gama * (S[1] + I[1] + R[1])  * A * (S[1] * ns[1] + I[1] * ni[1] * phi + R[1] * nr[1]))
    ...
    for (t in 2:H){
      ...
      #En Tr[t-1] cs = A * (1-taxn) * ns ; ci = A * (1 - taxn) * phi * ni
      
      #he[t-1] = 2 * 10^(3) * I[t-1] + 1
      pic[t-1] = pi1 * (1 + -0.8/(1 + exp(- 4 * (he[t-1]/20 - 0.8)))^2)
      pin[t-1] = pi2 * (1 + -0.8/(1 + exp(- 4 * (he[t-1]/20 - 0.8)))^2)
      
      Tr[t-1] = pic[t-1] * S[t-1] * (A * ns[t-1] - he[t-1]) * I[t-1] * (A * phi * ni[t-1] - he[t-1]) + pin[t-1] * S[t-1] * ns[t-1] * I[t-1] * ni[t-1] + pi3 * S[t-1] * I[t-1]
      
      #pid[t-1] = pidbar * (1 + (0.1 - 1.1)/((1 + 0.5 * exp(- 10 * (he[t-1]/40 - 0.4)))^2)) + kappa * I[t-1]^2
      ...
      #he[t] = 2000 * I[t]
      b[t] = (he[t-1] + (1/beta) * b[t-1] + (gama * b_ss - taxn_ss) * A * (S[t] * ns[t] + I[t] * ni[t] * phi + R[t] * nr[t]))/(1 + gama * A * (S[t] * ns[t] + I[t] * ni[t] * phi + R[t] * nr[t]))
      #b[t] = ((S[t] + I[t] + R[t])*(he[t-1] + (1/beta) * b[t-1]) + (gama * b_ss - taxn_ss) * A * (S[t] * ns[t] + I[t] * ni[t] * phi + R[t] * nr[t]))/((S[t] + I[t] + R[t]) + gama * (S[t] + I[t] + R[t]) * A * (S[t] * ns[t] + I[t] * ni[t] * phi + R[t] * nr[t]))
    ...
    #pid[length(pid)] = pidbar * (1 + (0.1 - 1.1)/((1 + 0.5 * exp(- 10 * (he[length(he)]/40 - 0.4)))^2)) + kappa * I[length(I)]^2
    pid[length(pid)] = pidbar + kappa * I[length(I)]^2
    pic[length(pic)] = pi1 * (1 - 0.8/(1 + exp(- 4 * (he[length(he)]/20 - 0.8)))^2)
    pin[length(pin)] = pi2 * (1 - 0.8/(1 + exp(- 4 * (he[length(he)]/50 - 0.8)))^2)
    #he[length(he)] = 1
    ...
    for (t in H:1){
      ...
      Ur[t] = log(cr[t]) - theta / 2 * nr[t]^2 + 0.3*log(he[t]) + beta * Ur[t+1]
      Ui[t] = log(ci[t]) - theta / 2 * ni[t]^2 + 0.3*log(he[t]) + (1 - pr_treat[t]) * beta * ((1 - pir - pid[t]) * Ui[t+1] + pir * Ur[t+1]) + beta * pr_treat[t] * Ur[t+1]
      Us[t] = log(cs[t]) - theta / 2 * ns[t]^2 + 0.3*log(he[t]) + (1 - pr_vacc[t]) * beta * ((1 - tau[t]) * Us[t+1] + tau[t] * Ui[t+1]) + pr_vacc[t] * beta * Ur[t+1]
      Utot[t] = S[t] * Us[t] + I[t] * Ui[t] + R[t] * Ur[t]
    }
    ...
    D_PIB = integer(H)
    for (t in 1:H){
      cs_comp[t] = A*(1-taxn[t])/(theta*ns[t]*(1+ctax[t])+mus[t]*I[t]*(pin[t]*ni[t]+pic[t]*A*(1-taxn[t])*ci[t]))
      cr_comp[t] = (A*nr[t] + transfer[t] + taxn[t]*A*(N[t]/(1-D[t]) - nr[t]) - he[t])/(1+ctax[t])
      D_PIB[t] = b[t]/(A*N[t])*100
    }
    
    return(list('ns'= ns, 'ni'= ni, 'nr'= nr, 'cs'= cs, 'ci'= ci, 'cr'= cr, 'transfer'= transfer, 'Tr'= Tr, 'S'= S, 'I'= I,
                'R'= R, 'D'= D, 'tau'= tau, 'Ur'= Ur[1:(length(Ur)-1)], 'Ui'= Ui[1:(length(Ui)-1)], 'Us'= Us[1:(length(Us)-1)],'Utot'= Utot,  'mus'= mus, 'lami'= lami, 'P'= P,
                ...
                'pi2'= pi2, 'pi3'= pi3, 'pic'= pic, 'pin'= pin, 'b'= b, 'b_ss'= b_ss, 'he'= he,'dp'= dp, 'taxn'= taxn, 'taxn_ss'= taxn_ss, 'D/PIB'= D_PIB, 'comprobacion1' = comprobacion1, 'cs_comp'= cs_comp, 'cr_comp'= cr_comp,'n_ss'= n_ss, 'c_ss'= c_ss))  }
  )
}
...
td_solve <- function(ctax, he, pr_treat, pr_vacc, pi1, pi2, pi3, eps, pidbar, pir, kappa, phi, 
  ...
  return(list('r' = results, 'H_U_factored' = H_U_factored))
  #return(results)
}
...
#tax_prueba = taxcEdward$par[1:H]

#td3.1 <- td_solve(integer(H), integer(H)+1, pr_treat=integer(H), pr_vacc=integer(H), pi1=pi1SM, pi2=pi2SM, pi3=pi3SM, 
...
#saveRDS(td3.1[["H_U_factored"]], "H_U_factored.opt.RDS")
