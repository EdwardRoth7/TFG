...
# Resuelve el problema de Ramsey y guarda el resultado en res
...
#taxcEdward <- ramsey(ctax0=integer(H), s0=1, i0=1, r0=1, pr_treat=integer(H), pr_vacc=integer(H), pi1=pi1SM, pi2=pi2SM, pi3=pi3SM,
#eps=eps, pidbar=pid, pir=pir, kappa=0.0, phi=phi, theta=theta, 
#A=A, beta=beta, maxit=100, h=1E-4, tol=1E-8, tol_ramsey=1E-3, noisy=FALSE) # Ramsey, SIR Macro

                                   #Solves Ramsey's problem. Any change in the parameters must be 
#tax_prueba = taxcEdward$par[1:H] #followed by a re-running of ramsey() function (H + 2 Jacobians + optim()/base/ optimization)


# Ahora deberías poder acceder a ctax y he en res
